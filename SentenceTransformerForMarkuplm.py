...
from .models import Pooling, Dense, MarkuplmTransformer
...
class SentenceTransformerForMarkuplm(nn.Sequential):
    """
    ...
    def encode(self, sentences: Union[str, List[str]],
        ...
        for start_index in trange(0, len(sentences), batch_size, desc="Batches", disable=not show_progress_bar):
            ...
            #features = self.process(sentences_batch)
    ...
    def start_multi_process_pool(self, target_devices: List[str] = None):
        ...
        for cuda_id in target_devices:
            p = ctx.Process(target=SentenceTransformerForMarkuplm._encode_multi_process_worker,
    ...
    def tokenize(self, texts: Union[List[str], List[Dict], List[Tuple[str, str]]]):
        ...
        Tokenizes the texts
        """
        print('tokenize_text(' + str(len(texts)) + '个):' + str(texts))
        #print('module:' + str(type(self._first_module())))
        return self._first_module().process(texts)
    ...
    def save(self, path: str, model_name: Optional[str] = None, create_model_card: bool = True,
        ...
        with open(os.path.join(path, 'config_sentence_transformers.json'), 'w') as fOut:  # 主文件夹下
            json.dump(self._model_config, fOut, indent=2)

        # Save modules
        for idx, name in enumerate(self._modules):
            module = self._modules[name]
            if idx == 0 and isinstance(module, MarkuplmTransformer):  # Save transformer model in the main folder
                model_path = path + "/"
            else:
                model_path = os.path.join(path, str(idx) + "_" + type(module).__name__)  # 0_MarkuplmTransformer
        ...
        with open(os.path.join(path, 'modules.json'), 'w') as fOut:  # 主文件夹下
    ...
    def _create_model_card(self, path: str, model_name: Optional[str] = None,
        ...
        if self._model_card_text is not None and len(self._model_card_text) > 0:
            ...
            if len(self._modules) == 2 and isinstance(self._first_module(), MarkuplmTransformer) and isinstance(
    ...
    def smart_batching_collate(self, batch):
        ...
        for example in batch:
            print('example' + str(i) + ':' + str(example))
            for idx, text in enumerate(example.texts):
                print('example_texts' + str(idx) + ':' + str(text))
    ...
    def fit(self,
        ...
        for dataloader in dataloaders:
            dataloader.collate_fn = self.smart_batching_collate  # 数据整理
    ...
    def _load_auto_model(self, model_name_or_path):
        ...
        logger.warning(
            "No sentence-transformers model found with name {}. Creating a new one with MEAN pooling.".format(
                model_name_or_path))
        transformer_model = MarkuplmTransformer(model_name_or_path)
    ...
    @staticmethod
    def load(input_path):
        return SentenceTransformerForMarkuplm(input_path)
